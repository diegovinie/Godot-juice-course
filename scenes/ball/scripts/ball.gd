...
var hitstop_bump_paddle = 5
var hitstop_bump_block = 5
var hitstop_bomb = 10
...
func _physics_process(delta: float) -> void:
	if hitstop_frames > 0: 
		hitstop_frames -= 1
		if hitstop_frames <= 0: stop_hitstop()
		return
...
#		print("Dot:", normal.dot(Vector2.UP))
		...
		if boost_factor == 1.0:
			start_hitstop(hitstop_bump_paddle)
			collision.get_collider().start_hitstop(hitstop_bump_paddle)
...
#				print("BELOW HALF")
		...
		if collision.get_collider().type == collision.get_collider().TYPE.ENERGY or \
			...
			start_hitstop(hitstop_bump_block)
	else:
...
func scale_based_on_velocity() -> void:
	...
	var max_deform = sprite_base_scale * Vector2(1.3, 0.6)
	sprite.scale = lerp(sprite_base_scale, max_deform, velocity / max_speed)
...
func color_based_on_velocity() -> void:
	...
	$SpeedParticles.self_modulate = lerp(Color.WHITE, max_speed_color, val)
...
func attract(g_position) -> void:
	attracted = true
	attracted_to = g_position
